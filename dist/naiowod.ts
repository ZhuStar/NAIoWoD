...
interface CardMap { [key: string]: CardValue }
type CardValue = string | number | boolean | null | CardValue[] | CardMap;
...
function parseCardText(text: string | undefined): CardValue | undefined {
...
function formatCardText(value: CardValue | undefined): string {
  if (value === undefined) return "";
  const out: string[] = [];
  if (isMap(value)) {
...
function inlineCardText(value: CardValue | undefined): string {
...
function asNumber(v: CardValue | undefined): number | undefined {
...
function asBool(v: CardValue | undefined): boolean | undefined {
  const own = ownValue(v);
...
function asText(v: CardValue | undefined, sep = ", "): string | undefined {
  const own = ownValue(v);
...
function asList(v: CardValue | undefined): CardValue[] {
  if (v === undefined || v === null || v === "") return [];
  if (Array.isArray(v)) return v;
  return [v];
}

function asStringList(v: CardValue | undefined): string[] {
  return asList(v).map(x => asText(x)).filter((x): x is string => x !== undefined && x.length > 0);
}

function asMap(v: CardValue | undefined): CardMap {
  return isMap(v) ? v : {};
}
...
function asNamedList(v: CardValue | undefined): Array<{ name: string; body: CardMap }> {
...
function canonicalCardText(value: CardValue | undefined): string {
...
interface ConditionResult extends ExprResult { truth: boolean }
function evaluateCondition(expr: string, scope: ExprScope): ConditionResult {
  const src = (expr ?? "").trim();
...
const BASE_CREATION: CreationBudget = {
...
const CREATION_SOURCES = ["freebies", "arcana"];
function sourceDrawsOnPurse(source: string | undefined): boolean {
  return source === undefined || CREATION_SOURCES.includes(StringUtil.normalize(source));
}
function grantSourceNote(source: string): string | undefined {
...
function describeCreationGrant(g: CreationGrant): string {
...
type BudgetEntry = string | BudgetDef;
function budgetDef(entry: BudgetEntry | undefined): BudgetDef {
...
function budgetBuyable(price: string | undefined): boolean {
  const p = (price ?? "").trim();
...
function capabilityNote(name: string): string | undefined {
...
const ARCANA_CAPABILITY = "arcana";
...
function capabilitiesOpenArcana(capabilities: string[]): boolean {
...
const REACTIONS: Record<string, () => DamageReaction> = {
...
function templateFromDef(def: TemplateDef, parent?: TemplateConfig): TemplateConfig {
...
const DEFAULT_TEMPLATE_DEFS: TemplateDef[] = [
  {
    name: "ouroboros",
    extends: "mage",
...
function makeTemplateDef(parts: Partial<TemplateDef> & { name: string }): TemplateDef {
...
function applyTemplateDefs(defs: TemplateDef[]): string[] {
...
function creationBudgetFor(keys: string[]): CreationBudget {
...
function fellowshipByName(name: string): (Fellowship & { id: string }) | undefined {
  const key = StringUtil.normalize(name);
...
function clanByName(name: string): (Clan & { id: string }) | undefined {
...
function clanFamilies(): Array<{ id: string; name: string }> {
...
function clanFamilyOf(name: string): string {
...
const GHOUL_FAMILIES: Record<string, Clan> = {};
const REVENANT_FAMILIES: Record<string, Clan> = {};
...
const AFFINITY_SOURCES: Array<{ choice: string; label: string; families: Record<string, Clan> }> = [
...
function familyByName(families: Record<string, Clan>, name: string): (Clan & { id: string }) | undefined {
  const key = StringUtil.normalize(name);
...
// =============================================================================
interface SupernaturalCategory {
  name: string;
  label?: string;
...
interface SupernaturalTraitDef {
  name: string;
  category: string;
...
const DEFAULT_SUPERNATURAL_CATEGORIES: SupernaturalCategory[] = [
...
const DEFAULT_SUPERNATURAL_TRAITS: SupernaturalTraitDef[] = [
...
function supernaturalTraitOf(name: string, defs: SupernaturalTraitDef[] = DEFAULT_SUPERNATURAL_TRAITS): SupernaturalTraitDef | undefined {
  const key = StringUtil.normalize(name);
  return defs.find(d => StringUtil.normalize(d.name) === key);
}

// Is this category open to these templates?
function categoryOpenTo(cat: SupernaturalCategory, templates: string[]): boolean {
...
// =============================================================================
type CostPurse = "experience" | "freebie" | "maturation";
const COST_PURSES: CostPurse[] = ["experience", "freebie", "maturation"];
type CostTable = Record<string, Record<string, string>>;

const DEFAULT_ADVANCEMENT_COSTS: CostTable = {
...
function advancementCostsFrom(overrides: CostTable): CostTable {
  const out: CostTable = {};
...
type ArcanumKind = "arcanum" | "taint";
...
type OwnedPowerKind = MeritFlawKind | ArcanumKind;
const MERIT_FLAW_KINDS: MeritFlawKind[] = ["merit", "flaw"];
const ARCANUM_KINDS: ArcanumKind[] = ["arcanum", "taint"];
const OWNED_POWER_KINDS: OwnedPowerKind[] = [...MERIT_FLAW_KINDS, ...ARCANUM_KINDS];
function isArcanumKind(kind: string): kind is ArcanumKind {
  return (ARCANUM_KINDS as string[]).includes(kind);
}
function isMeritFlawKind(kind: string): kind is MeritFlawKind {
...
function budgetOfKind(def: { kind: OwnedPowerKind; budget?: string }): string {
  return StringUtil.normalize(def.budget ?? KIND_BUDGET[def.kind] ?? "freebie");
}
function kindSpends(kind: OwnedPowerKind): boolean { return KIND_SPENDS[kind] ?? true; }
...
interface MeritFlawDef extends OwnedPowerDef { kind: MeritFlawKind }
...
interface ArcanumDef extends OwnedPowerDef { kind: ArcanumKind }
...
function meritCostFor(def: OwnedPowerDef, templates: string[]): {
...
function instanceLimitsOf(def: OwnedPowerDef): InstanceLimit[] {
  return [...(def.limits ?? [])];
}
...
function resolvePowerInstance<T extends OwnedPowerDef>(
...
function passiveOpsOf(def: OwnedPowerDef, param: string | undefined, points: number): EffectOp[] {
...
function parsePassiveOps(raw: string): EffectOp[] | { error: string } {
  const text = raw.trim();
  if (text.startsWith("[")) {
    try {
...
function describePassiveOp(op: EffectOp): string {
...
function effectOpFromCard(raw: CardValue): EffectOp | undefined {
...
function effectOpsFromCard(raw: CardValue | undefined): EffectOp[] {
...
function arcanumFromCard(name: string, body: CardMap): ArcanumDef | undefined {
...
function kindOnCard(body: CardMap): string | undefined {
...
const EXCLUSIVE_MERITS_FLAWS: MeritFlawDef[] = [
...
// =============================================================================
const DEFAULT_ARCANA: ArcanumDef[] = [
  {
...
type ConstraintDomain = "background" | "merit" | "flaw" | "meritflaw" | "arcanum" | "any";
...
const CONSTRAINT_DOMAINS: ConstraintDomain[] = ["background", "merit", "flaw", "meritflaw", "arcanum", "any"];
...
type AfflictionMode = "passive" | "togglable" | "temporary";
const AFFLICTION_MODES: AfflictionMode[] = ["passive", "togglable", "temporary"];
...
function expiryElapsed(expiry: AfflictionExpiry, now: number, condition?: boolean): boolean {
...
function stripSys(reply: string): string {
...
function sysNote(reply: string, note: string): string {
  const m = /^\[SYSTEM: ([\s\S]*)\]$/.exec(reply.trim());
...
function configEntryText(header: string[], data: CardValue): string {
...
function namedDefsToCard<T extends { name: string }>(defs: T[]): CardMap {
  const out: CardMap = {};
...
function parseConfigBody(text: string | undefined): CardValue | undefined {
...
function parseNamedConfigList<T extends { name: string }>(parsed: CardValue | undefined): Array<Partial<T> & { name: string }> {
...
async function ensurePath(virtualPath: string, generalHeader: string[] = ["Data for this category, below the marker."]): Promise<{ category: string; createdEntry: boolean }> {
...
const MeritFlawRegistry = new OwnedPowerRegistry<MeritFlawDef>(
  DEFAULT_MERITS_FLAWS, "srd:merits-flaws", meritFlawFromCard, "MERITS",
  MERIT_FLAW_KINDS, "srd:arcana",
);
const ArcanumRegistry = new OwnedPowerRegistry<ArcanumDef>(
...
function migratePowerBuckets(char: PlayableCharacter): PlayableCharacter {
...
function characterToCard(char: PlayableCharacter): CardMap {
...
function characterFromCard(raw: CardValue | undefined): PlayableCharacter | undefined {
...
interface OwnedPowerInstance<T extends OwnedPowerDef = OwnedPowerDef> {
  key: string;
  def: T;
  param?: string;
  points: number;
...
function ownedMeritInstances(char: PlayableCharacter): Array<OwnedPowerInstance<MeritFlawDef>> {
  return instancesOf(char.meritsFlaws, n => MeritFlawRegistry.get(n));
}
function ownedArcanumInstances(char: PlayableCharacter): Array<OwnedPowerInstance<ArcanumDef>> {
  return instancesOf(char.arcana, n => ArcanumRegistry.get(n));
}
function ownedPowerInstances(char: PlayableCharacter): OwnedPowerInstance[] {
...
function savedRollToCard(roll: SavedRoll): CardMap {
...
function savedRollFromCard(body: CardMap): SavedRoll | undefined {
...
const TEMPLATES_ENTRY = "wod:config:templates";
let lastTemplateProblems: string[] = [];
const TemplateRegistry = new ListConfigStore<TemplateDef>({
...
const COSTS_CONFIG_ENTRY = "wod:config:costs";
const AdvancementCosts = new MapConfigStore<Record<string, string>>({
...
interface ArmedCooldown { expiry: AfflictionExpiry; at: number }
...
interface ResourceView { def: ResourceDef; current: number; max: number; blocked: string[] }
